...
const productInclude={images:true,variants:true,brand:true,category:true};
async function adminProductPayload(input) {
  const name=String(input.name||'').trim(), sku=String(input.sku||'').trim(), slug=String(input.slug||'').trim();
  if(!name||!sku||!slug) throw Object.assign(new Error('نام، کد کالا و نامک محصول الزامی است'),{status:400});
  const category=await prisma.category.findUnique({where:{slug:String(input.category||'')}});
  if(!category) throw Object.assign(new Error('دسته‌بندی انتخاب‌شده معتبر نیست'),{status:400});
  const brandName=String(input.brand||'مایا آزما').trim();
  let brand=await prisma.brand.findUnique({where:{name:brandName}});
  if(!brand) brand=await prisma.brand.create({data:{name:brandName,slug:`brand-${Date.now()}-${Math.random().toString(36).slice(2,6)}`}});
  const images=Array.isArray(input.images)?input.images.filter(Boolean).map((url,order)=>({url:String(url),order})):[];
  const variants=Array.isArray(input.variants)?input.variants.filter(v=>v?.name).map((v,index)=>({sku:`${sku}-V${index+1}-${Date.now()}`,price:v.price==null?null:Number(v.price),stock:Number(v.stock||0),attributes:{name:String(v.name),type:v.type||'گزینه',value:v.value||v.name}})):[];
  return {name,sku,slug,description:String(input.shortDescription||input.description||''),price:Number(input.price||0),discount:Math.max(0,Math.min(100,Number(input.discount||0))),stock:Math.max(0,Number(input.stock||0)),status:input.active===false?'DRAFT':'ACTIVE',specifications:input.specifications||input.specs||{},categoryId:category.id,brandId:brand.id,images:{create:images},variants:{create:variants}};
}
app.get('/api/admin/catalog', auth(['ADMIN']), async(_req,res,next)=>{try{ok(res,await prisma.product.findMany({include:productInclude,orderBy:{updatedAt:'desc'}}));}catch(e){next(e);}});
app.post('/api/admin/products', auth(['ADMIN']), async(req,res,next)=>{try{ok(res,await prisma.product.create({data:await adminProductPayload(req.body),include:productInclude}),201);}catch(e){next(e);}});
app.put('/api/admin/products/:id', auth(['ADMIN']), async(req,res,next)=>{try{const data=await adminProductPayload(req.body);const {images,variants,...product}=data;await prisma.productImage.deleteMany({where:{productId:req.params.id}});await prisma.productVariant.deleteMany({where:{productId:req.params.id}});ok(res,await prisma.product.update({where:{id:req.params.id},data:{...product,images,variants},include:productInclude}));}catch(e){next(e);}});
app.delete('/api/admin/products/:id', auth(['ADMIN']), async(req,res,next)=>{try{await prisma.product.delete({where:{id:req.params.id}});ok(res,{deleted:true});}catch(e){next(e);}});
app.get('/api/admin/categories', auth(['ADMIN']), async(_req,res,next)=>{try{ok(res,await prisma.category.findMany({orderBy:{order:'asc'},include:{parent:true}}));}catch(e){next(e);}});
app.post('/api/admin/categories', auth(['ADMIN']), async(req,res,next)=>{try{const parent=req.body.parentSlug?await prisma.category.findUnique({where:{slug:String(req.body.parentSlug)}}):null;ok(res,await prisma.category.create({data:{name:String(req.body.name||'').trim(),slug:String(req.body.slug||'').trim(),parentId:parent?.id||null,active:req.body.active!==false,order:Number(req.body.order||0)}}),201);}catch(e){next(e);}});
app.put('/api/admin/categories/:id', auth(['ADMIN']), async(req,res,next)=>{try{const parent=req.body.parentSlug?await prisma.category.findUnique({where:{slug:String(req.body.parentSlug)}}):null;ok(res,await prisma.category.update({where:{id:req.params.id},data:{name:String(req.body.name||'').trim(),slug:String(req.body.slug||'').trim(),parentId:parent?.id||null,active:req.body.active!==false,order:Number(req.body.order||0)}}));}catch(e){next(e);}});
app.delete('/api/admin/categories/:id', auth(['ADMIN']), async(req,res,next)=>{try{await prisma.category.delete({where:{id:req.params.id}});ok(res,{deleted:true});}catch(e){next(e);}});
...
app.post('/api/cart/items', auth(), async(req,res,next)=>{try{const quantity=Number(req.body.quantity||1);if(!Number.isInteger(quantity)||quantity<1)return fail(res,'تعداد نامعتبر است');const product=await prisma.product.findUnique({where:{id:req.body.productId}});if(!product||product.stock<quantity)return fail(res,'موجودی محصول کافی نیست',409);const cart=await prisma.cart.upsert({where:{userId:req.auth.sub},create:{userId:req.auth.sub},update:{}}),variantId=req.body.variantId||null;const existing=await prisma.cartItem.findFirst({where:{cartId:cart.id,productId:product.id,variantId}});const item=existing?await prisma.cartItem.update({where:{id:existing.id},data:{quantity:{increment:quantity}}}):await prisma.cartItem.create({data:{cartId:cart.id,productId:product.id,variantId,quantity}});ok(res,item,201);}catch(e){next(e);}});
...
app.get('/api/orders', auth(), async(req,res,next)=>{try{ok(res,await prisma.order.findMany({where:req.auth.role==='ADMIN'?{}:{userId:req.auth.sub},include:{items:true,address:true,user:{select:{firstName:true,lastName:true,email:true,mobile:true}}},orderBy:{createdAt:'desc'}}));}catch(e){next(e);}});
