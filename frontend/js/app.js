/* ============================================================
   Maya Azma storefront — application core (redesigned 2026)
   Data layer + hash router + page templates.
   ============================================================ */
...
const defaultSliders = [
  {
    ...
    image: "assets/images/hero-equipment.jpg",
  ...
  },
  {
    ...
    image: "assets/images/hero-cellculture.jpg",
  ...
  },
  {
    ...
    image: "assets/images/hero-molecular.jpg",
...
const fallbackImages = {
  "laboratory-equipment": "assets/images/hero-equipment.jpg",
  "cell-culture": "assets/images/hero-cellculture.jpg",
  molecular: "assets/images/hero-molecular.jpg",
  "laboratory-consumables": "assets/images/hero-cellculture.jpg",
  "microbial-culture": "assets/images/hero-molecular.jpg",
  "miscellaneous-lab-supplies": "assets/images/hero-equipment.jpg",
};
const productImages = (p) => {
  ...
  return [
    ...new Set(
      list.length
        ? list
        : [fallbackImages[p.category] || "assets/images/hero-equipment.jpg"],
    ),
  ];
};
function renderMega() {
...
function card(p) {
  ...
  return `<article class="card">${p.discount ? `<span class="badge">${fa(p.discount)}٪ تخفیف</span>` : ""}<button class="card-wish ${wished ? "active" : ""}" data-wish="${p.id}" aria-label="افزودن به علاقه‌مندی‌ها"><i data-lucide="heart"></i></button><div class="card-image" data-product="${p.id}"><img src="${img}" alt="${p.name}" loading="lazy"></div><div class="card-body"><div class="card-brand"><strong>${p.brand}</strong><span>${p.sku}</span></div><h3 data-product="${p.id}">${p.name}</h3><div class="rating-row"><span class="rating">★ ${fa(p.rating || 4.8)}</span><span class="availability ${available ? "" : "out"}">${available ? "موجود در انبار" : "ناموجود"}</span></div><div class="price-area">${p.discount ? `<div class="old">${money(p.price)}</div>` : '<div class="old">&nbsp;</div>'}<div class="card-actions"><strong>${money(finalPrice(p))}</strong><button class="add-cart" data-add="${p.id}" ${!available ? "disabled" : ""} aria-label="افزودن به سبد خرید"><i data-lucide="shopping-bag"></i></button></div></div></div></article>`;
}
/* ---------- hero slider ---------- */
let sliderIndex = 0,
  sliderTimer;
...
function bindSlider() {
  ...
  sliderTimer = setInterval(() => go(sliderIndex + 1), 6000);
}
/* ---------- pages ---------- */
function home() {
  const offers = products.filter((p) => p.active !== false && p.discount);
  return (
    sliderMarkup() +
    `<section class="benefits">${["تضمین اصالت|کالای معتبر", "ارسال تخصصی|بسته‌بندی ایمن", "مشاوره علمی|کارشناسان مجرب", "ضمانت بازگشت|تا ۷ روز"].map((x) => { const [b, s] = x.split("|"); return `<article><i>✓</i><b>${b}</b><small>${s}</small></article>`; }).join("")}</section><section class="section"><div class="section-head"><div><span class="section-kicker">دسترسی سریع</span><h2>دسته‌بندی محصولات</h2></div><a href="#products">مشاهده همه ←</a></div><div class="category-grid">${categories.map((c) => `<a class="category-card" href="#products?cat=${c[0]}"><i>${c[2]}</i><b>${c[1]}</b><small>${fa(products.filter((p) => p.category === c[0]).length)} محصول</small></a>`).join("")}</div></section><section class="section alt product-section"><div class="section-head"><div><span class="section-kicker">انتخاب اقتصادی آزمایشگاه‌ها</span><h2>پیشنهادهای ویژه امروز</h2><p class="product-subtitle">محصولات منتخب با تضمین اصالت و قیمت رقابتی</p></div><a class="view-all" href="#products?discount=1">مشاهده همه ←</a></div><div class="grid">${offers.slice(0, 4).map(card).join("")}</div></section><section class="section"><div class="section-head"><div><span class="section-kicker">تازه‌های مایا آزما</span><h2>محصولات جدید و تخصصی</h2><p class="product-subtitle">انتخاب‌شده برای پژوهشگران، دانشگاه‌ها و آزمایشگاه‌های حرفه‌ای</p></div><a class="view-all" href="#products">فروشگاه کامل ←</a></div><div class="grid">${products.filter((p) => p.active !== false).slice(4, 12).map(card).join("")}</div></section>`
  );
}
function productsPage(params) {
  ...
  const catLabel = cat ? categories.find((c) => c[0] === cat)?.[1] : "";
  return `<div class="page-hero"><div class="breadcrumb"><a href="#home">خانه</a> / محصولات${catLabel ? " / " + catLabel : ""}</div><h1>${catLabel || "محصولات آزمایشگاهی"}</h1><p>${fa(list.length)} کالا برای انتخاب شما</p></div><div class="catalog"><aside class="filters"><h3><i data-lucide="settings"></i> فیلتر و مرتب‌سازی</h3><div class="field"><label for="filterCat">دسته‌بندی</label><select id="filterCat"><option value="">همه دسته‌ها</option>${categories.map((c) => `<option value="${c[0]}" ${cat === c[0] ? "selected" : ""}>${c[1]}</option>`).join("")}</select></div><div class="field"><label for="filterBrand">برند</label><select id="filterBrand"><option value="">همه برندها</option>${[...new Set(products.map((p) => p.brand))].map((x) => `<option ${brand === x ? "selected" : ""}>${x}</option>`).join("")}</select></div><div class="check-row"><input id="onlyStock" type="checkbox" ${params.get("stock") ? "checked" : ""}><label for="onlyStock">فقط کالاهای موجود</label></div><div class="check-row"><input id="onlyDiscount" type="checkbox" ${params.get("discount") ? "checked" : ""}><label for="onlyDiscount">فقط تخفیف‌دارها</label></div><button class="btn" id="applyFilters">اعمال فیلتر</button></aside><section><div class="catalog-head"><p><b>${fa(list.length)}</b> نتیجه${q ? ` برای «${q}»` : ""}</p><select id="sort" aria-label="مرتب‌سازی"><option value="new">جدیدترین</option><option value="cheap" ${sort === "cheap" ? "selected" : ""}>ارزان‌ترین</option><option value="expensive" ${sort === "expensive" ? "selected" : ""}>گران‌ترین</option></select></div>${list.length ? `<div class="grid">${list.map(card).join("")}</div>` : `<div class="empty"><i>⌕</i><h2>محصولی پیدا نشد</h2><p>فیلترها یا عبارت جستجو را تغییر دهید.</p></div>`}</section></div>`;
}
function productPage(id) {
  ...
  return `<div class="page-hero"><div class="breadcrumb"><a href="#home">خانه</a> / <a href="#products">محصولات</a> / ${p.name}</div></div><section class="product-detail"><div><div class="gallery-main"><img id="mainProductImage" src="${imgs[0]}" alt="${p.name}"></div><div class="thumbnails">${imgs.map((img, i) => `<button class="${i === 0 ? "selected" : ""}" data-thumb="${img}" aria-label="تصویر ${i + 1}"><img src="${img}" alt=""></button>`).join("")}</div></div><div class="detail-info"><span class="section-kicker">${p.brand}</span><h1>${p.name}</h1><p>کد محصول: <b>${p.sku}</b></p><div class="rating">★★★★★ ${fa(p.rating || 4.8)} از ۵</div><p>${short}</p><div class="stock">${p.stock ? `✓ موجود در انبار (${fa(p.stock)} عدد)` : "ناموجود"}</div><div class="detail-price">${money(finalPrice(p))}</div><b>نوع / بسته‌بندی:</b><div class="variant-list">${(p.variants || ["۱ عدد"]).map((v, i) => `<button class="${i === 0 ? "selected" : ""}">${typeof v === "string" ? v : v.name}</button>`).join("")}</div><div class="buy-row"><div class="qty"><button data-qty="down" aria-label="کاهش">−</button><span id="detailQty">1</span><button data-qty="up" aria-label="افزایش">+</button></div><button class="btn" data-add="${p.id}"><i data-lucide="shopping-cart"></i> افزودن به سبد خرید</button><button class="btn outline" data-wish="${p.id}" aria-label="علاقه‌مندی"><i data-lucide="heart"></i></button></div><div class="product-trust"><div>✓ ضمانت اصالت</div><div>🚚 ارسال ایمن</div><div>☎ مشاوره تخصصی</div></div></div></section><section class="tabs"><div class="tab-buttons"><button class="active" data-tab="desc">توضیحات کامل</button><button data-tab="spec">مشخصات فنی</button><button data-tab="use">کاربردها و نگهداری</button><button data-tab="review">نظرات کاربران</button></div><div id="tabContent" class="tab-content">${long}</div></section>`;
}
function cartPage() {
...
function accountPage() {
  ...
  return `<div class="page-hero"><h1>سلام ${state.user.firstName}</h1><p>به پنل کاربری مایا آزما خوش آمدید.</p></div><div class="account-layout"><aside class="sidebar"><a class="active">نمای کلی</a><a href="#orders">سفارش‌های من</a><a href="#wishlist">علاقه‌مندی‌ها</a><a>آدرس‌ها</a>${state.user.role === "ADMIN" ? '<a href="#admin">پنل مدیریت</a>' : ""}<button id="logout">خروج از حساب</button></aside><section><div class="stats"><div class="stat"><small>سفارش‌ها</small><strong>${fa(state.orders.length)}</strong></div><div class="stat"><small>علاقه‌مندی‌ها</small><strong>${fa(state.wish.length)}</strong></div><div class="stat"><small>اعتبار</small><strong>۰ تومان</strong></div></div><div class="panel" style="margin-top:20px"><h2>اطلاعات شخصی</h2><div class="form-grid"><div><small>نام و نام خانوادگی</small><p>${state.user.firstName} ${state.user.lastName || ""}</p></div><div><small>ایمیل</small><p>${state.user.email}</p></div></div></div></section></div>`;
}
function ordersPage() {
...
function trackingPage(params) {
  ...
  return `<div class="page-hero"><h1>پیگیری سفارش</h1><p>کد سفارش را وارد کنید تا وضعیت واقعی آن نمایش داده شود.</p></div><section class="section"><div class="panel" style="max-width:760px;margin:auto"><form id="trackForm" class="coupon"><input id="trackCode" required value="${id}" autocomplete="off" placeholder="کد سفارش مانند MAYA-12345678"><button>پیگیری</button></form>${!id ? '<div class="admin-empty">برای مشاهده وضعیت، کد سفارش خود را وارد کنید.</div>' : !order ? '<div class="empty"><h2>سفارشی با این کد پیدا نشد</h2><p>کد را بررسی و دوباره تلاش کنید.</p></div>' : order.status === "CANCELLED" ? '<div class="empty"><h2>این سفارش لغو شده است</h2></div>' : `<h3>سفارش ${order.number}</h3><p style="margin:8px 0 4px">وضعیت فعلی: <span class="status-pill">${order.status}</span></p><div class="timeline">${steps.map((x, i) => `<div class="timeline-item ${i > level ? "pending" : ""}"><b>${x}</b><p>${i <= level ? "انجام شده" : "در انتظار انجام"}</p></div>`).join("")}</div>`}</div></section>`;
}
...
function render() {
  ...
  if (path === "admin" && params.get("section") === "orders") setTimeout(() => window.renderPersistentOrders?.(), 0);
}
/* ---------- actions ---------- */
