...
async function renderPersistentOrders() {
  ...
  try {
    ...
    host.innerHTML = `<div class="admin-toolbar"><div><h2>مدیریت سفارش‌ها</h2><small>${orders.length} سفارش واقعی · آخرین بروزرسانی ${new Date().toLocaleTimeString('fa-IR',{hour:'2-digit',minute:'2-digit'})}</small></div><button class="btn outline" data-refresh-orders>بروزرسانی</button></div>${orders.length ? `<div class="admin-order-list">${orders.map(order => `<article class="admin-order-card"><div class="admin-order-head"><div><small>شماره سفارش</small><strong>${order.orderNumber}</strong></div><a class="admin-invoice-link" href="#invoice?number=${encodeURIComponent(order.invoiceNumber || order.orderNumber)}">مشاهده فاکتور</a></div><div class="admin-order-details"><div><small>مشتری</small><b>${order.user?.firstName || '—'} ${order.user?.lastName || ''}</b><span>${order.user?.email || ''}</span></div><div><small>تاریخ</small><b>${new Date(order.createdAt).toLocaleDateString('fa-IR')}</b></div><div><small>مبلغ</small><b>${money(Number(order.total))}</b></div><label><small>وضعیت سفارش</small><select data-order-status="${order.orderNumber}">${Object.entries(faAdminStatus).map(([value,label]) => `<option value="${value}" ${order.status === value ? 'selected' : ''}>${label}</option>`).join('')}</select></label></div></article>`).join('')}</div>` : '<div class="admin-empty">هنوز سفارشی ثبت نشده است.</div>'}`;
...
setInterval(() => { if (location.hash.startsWith('#admin?section=orders') && !document.querySelector('[data-order-status]:focus')) renderPersistentOrders(); }, 60000);
